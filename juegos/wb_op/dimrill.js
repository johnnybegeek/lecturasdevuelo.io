/** Cosmos Manager - v1.0 | MIT License */
const WB = {
  state: null,
  config: { storageKey: 'dimrillState', toastDuration: 3000 },
  ...
  exportState() {
    try {
      ...
      a.href = u;
      a.download = `cosmos_${this.state.conceit.replace(/\s+/g, '_')}_${new Date().toISOString().split('T')[0]}.json`;
  ...
  renderConceptInput() {
    ...
    if (input) input.value = this.state.conceit;
  },
  ...
  toggleActionsMenu() {
    ...
    if (actionsButtons) {
      actionsButtons.classList.toggle('visible');
    }
  },
  ...
  handleScroll() {
    ...
    if (window.scrollY > 50) {
      ...
      if (actionsContainer) actionsContainer.style.bottom = '1rem';
    } else {
      if (mobileHeader) mobileHeader.classList.remove('hide');
      if (actionsContainer) actionsContainer.style.bottom = '2rem';
    }
  },
  ...
  setupListeners() {
    ...
    if (conceptInput) {
      conceptInput.addEventListener('input', e => this.updateConcept(e.target.value));
    }

    // Botones de exportar/importar/reset
    const exportBtn = document.getElementById('exportBtn');
    const importBtn = document.getElementById('importBtn');
    const fileInput = document.getElementById('fileInput');
    const resetBtn = document.getElementById('resetBtn');
    const floatingActionsBtn = document.getElementById('floatingActionsBtn');
    ...
    if (floatingActionsBtn) {
      floatingActionsBtn.addEventListener('click', () => this.toggleActionsMenu());
    }
    ...
    document.addEventListener('click', (e) => {
      ...
      if (actionsButtons && floatingActionsBtn) {
        if (!actionsButtons.contains(e.target) && !floatingActionsBtn.contains(e.target)) {
          actionsButtons.classList.remove('visible');
        }
      }
    });
  },
